...
- 검증: **단위 592 / 통합(e2e) 40 테스트 전부 통과**, API·Web `tsc` 0 error, 크롬 브라우저 실동작 확인.
...
## 9. 남은 권고 (향후 과제 — SYSTEM_DESIGN §6.6)
1. 스키마 FK 정책 명시화: `Document.form` `Cascade`→`Restrict`, `ApprovalLine.sharedLineRef` `SetNull` 명시.
2. 근무일정 확정(CONFIRMED) 정책 명문화(근태는 결재로도 차단 / 근무일정은 결재가 정식 변경 경로 — 비대칭).
3. 결재 규칙 스냅샷(`Request.ruleId`)으로 진행 중 요청 소급 방지.
4. 비활성 마스터 데이터 조회 필터 일원화(`Position.isActive` 등), 조직 계층 순환참조 검출.
