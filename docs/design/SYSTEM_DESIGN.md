...
4. 조회/계층 무결성:
   ...
   - ⏳ **비활성 마스터 데이터 조회 필터 일관성** (설계 판단 보류): `Position.isActive`/`CustomRequestType.isActive` 등 비활성 참조 노출. **선택 목록(드롭다운)은 active만 노출**, **이력/현재 배정 표시는 비활성도 보존**이라는 맥락 구분이 필요 — 일괄 필터링은 "직무 없는 직원"처럼 잘못 표기될 회귀 위험이 있어 엔드포인트별 용도 확정 후 적용.
