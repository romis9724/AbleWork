...
| A3 | 기안 양식 선택(양식함) | PAGE(카드 그리드) | Dialog 내 select | 양식함 카드 그리드 페이지 → 작성 페이지 라우팅 |
| A4 | 기안 작성/상신 | PAGE(풀페이지) | Dialog sm | `/me/documents/compose`·`/[id]/edit` 페이지 승격 |
| A5 | 기안 상세 | PAGE(하단 목록/결재 푸터) | Dialog sm(DocumentDetailDialog) | `/approval/documents/[id]` 상세 페이지 승격 |
...
- 권장 처리 순서: **A4/A5(작성·상세 PAGE) → C1/C2(결재·반송 팝업) → C3~C7(결재선 설정 팝업) → A1/A2(양식 등록·수정 페이지) → B(문서함 IA).**
