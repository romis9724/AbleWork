...
- ✅ C3 결재선 설정(조직 트리 선택) 팝업(PR#47) — `ApprovalLineDialog`: 좌측 조직 트리(부서 체크박스+직원 리프)+검색, 중앙 [결재][협조][수신][참조][공람] 추가 버튼(직원=개인 역할, 부서=협조→DEPT_COLLABORATOR/수신→DEPT_RECEIVER), 우측 결재선명 select+기안 고정행+단계 리스트(순서/삭제), [취소][적용]. DocumentComposeForm은 인라인 빌더→요약 카드+[결재선 설정] 버튼. 브라우저 E2E 검증 완료.
- ✅ C4/C6/C7 공람·참조 **사후 추가**(PR#50) — BE `POST /documents/:id/cc`(`addCcSteps`: 진행중/완료 문서에 VIEWER/REFERENCE step append, 비차단 PENDING, 권한=기안자/결재참여자, 중복·상태·권한 가드 + 단위테스트 5) + FE `AddCcDialog`(공람/참조 라디오 + 직원 멀티선택) → `DocumentDetailView` 푸터 [공람·참조 추가] 버튼. · C5 상위 결재선 변경(enable_upper_line_change) 잔여.
...
- 권장 처리 순서: ✅ **A3/A4(양식함 진입·작성 PAGE)** → ✅ **A5(상세 PAGE)** → ✅ **C1/C2(결재·반송 팝업)** → ✅ **C3(결재선 설정 팝업)** → ✅ **A1/A2(양식 등록·수정 페이지)** → ✅ **C4/C6/C7(공람/참조 사후지정)** → ✅ **B(문서함 IA)**. 잔여: C5 상위 결재선 변경.
